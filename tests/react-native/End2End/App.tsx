...
//@ts-ignore
AWS_SMOKE_TEST_BUCKET } from "react-native-dotenv";
const App = memo(() => {
  const [responseContent, setResponseContent] = useState("");
  const [uploadId, setUploadId] = useState("");
  const [uploadParts, setUploadParts] = useState<Array<{
    ETag: string;
    PartNumber: number;
  }>>([]);
  const s3 = new S3({
  ...
  return <>
      ...
      <SafeAreaView>
        <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.scrollViewContainer}>
          <Fragment>
            <Button title="S3.PutObject" testID="s3PutObject" onPress={async () => {
            ...
            <Button title="S3.getObject" testID="s3GetObject" onPress={async () => {
            const res = await s3.getObject({
              Bucket: AWS_SMOKE_TEST_BUCKET,
              Key
            });
            if (!(res.Body instanceof Blob)) throw new Error(`Expected Blob payload but got ${res.Body.constructor}`);
            ...
            <Button title="S3.listObjects" testID="s3ListObjects" onPress={async () => {
            const res = await s3.listObjects({
              Bucket: AWS_SMOKE_TEST_BUCKET
            });
            setResponseContent(JSON.stringify(res));
          }} />
            <Button title="S3.createMultipartUpload" testID="s3CreateMultipartUpload" onPress={async () => {
            ...
            <Button title="S3.uploadPart" testID="s3UploadPart" onPress={async () => {
            ...
            <Button title="S3.listParts" testID="s3ListParts" onPress={async () => {
            ...
            <Button title="S3.completeMultipartUpload" testID="s3CompleteMultipartUpload" onPress={async () => {
            ...
            <Button title="S3.headObject" testID="s3HeadObject" onPress={async () => {
            ...
            <Button title="S3.deleteObject" testID="s3DeleteObject" onPress={async () => {
